...
final class EachEntityIsAValueInAHashBinaryRedisRepositoryTests extends RedisTestContainer {

    static final Logger logger = LoggerFactory.getLogger(EachEntityIsAValueInAHashBinaryRedisRepositoryTests.class);
    ...
    class TestsForGet {
        ...
        void testGetEmptyEntity() {
            final var expectedPerson = Person.random();
            jedis.hset(SafeEncoder.encode("people"), SafeEncoder.encode(expectedPerson.getId()), new byte[0]);
            final var actualResult = repository.get(randomString());
            assertTrue(actualResult.isEmpty());
        }
    ...
    class TestsForGetMultiple {
        ...
        void testGetMultiple() {
            ...
            expectedPeopleMap.values().forEach(EachEntityIsAValueInAHashBinaryRedisRepositoryTests::insert);
    ...
    class TestsForGetAllIds {
        ...
        void testGetAllIds() {
            ...
            jedis.hdel(SafeEncoder.encode("people"), allIds.stream().map(SafeEncoder::encode).toArray(byte[][]::new));
    ...
    private static void insert(final Person person) {
        jedis.hset(SafeEncoder.encode("people"), SafeEncoder.encode(person.getId()), serializer.apply(person));
    }
