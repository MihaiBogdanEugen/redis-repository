...
public abstract class BaseStringHashClusterRedisRepository<T>
        extends ClusterRedisRepository
        implements StringHashClusterRedisRepository<T> {
     ...
     */
    public BaseStringHashClusterRedisRepository(final ClusterRedisRepositoryConfiguration configuration) {
    ...
    public final void set(final String id, final T entity) {
        ...
        final var key = getKey(id);
        runClusterCommand(key, jedis -> jedis.hset(key, convertTo(entity)));
    }
    ...
    public final void setField(final String id, final Map.Entry<String, String> fieldAndValue) {
        ...
        final var key = getKey(id);
        runClusterCommand(key, jedis -> jedis.hset(key, fieldAndValue.getKey(), fieldAndValue.getValue()));
    }
    ...
    public final void setFieldIfNotExists(final String id, final Map.Entry<String, String> fieldAndValue) {
        ...
        throwIfNull(fieldAndValue, "fieldAndValue");
        final var key = getKey(id);
        runClusterCommand(key, jedis -> jedis.hsetnx(key, fieldAndValue.getKey(), fieldAndValue.getValue()));
    }
